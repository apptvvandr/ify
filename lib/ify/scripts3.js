...
// 					}
// 				});
		this.player = new MediaElementPlayer(object, {
            //features: ['playpause'],
            features: [''],
            ...
            //audioHeight: 0
            success: function (mediaElement, domObject) {
              console.log("Callback: element is:")
              console.log(mediaElement)
              },
            error: function() {
              alert("Player can't be loaded")
            }
        }).media
	...
	this.playFile = function (file) {
		this.player.setSrc(file);
		this.player.play();
	}

    this.toggleLecture =function () {
...
$('#content-playlist-table > tbody').on('click', 'tr',
		function (e) {
			...
			player.playFile("../media.php?m=as&a=" + $(this).children("td").html())
			}
		)
//var player = new MediaElementPlayer(document.getElementById("player1"));
// ... more code ...
//player.pause();
//player.setSrc('http://alpha/ify-dev/media.php?m=as&a=G0wJmex6k1hB');
//player.play();

// Initialisation
/////////////////
var glob_rel_path = new Path("") ;
var player = new IfyPlayer(document.getElementById("player1"));
